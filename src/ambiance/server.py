    ...
    def do_POST(self) -> None:  # noqa: N802 - stdlib signature
        ...
        try:
            ...
            if path == "/api/vst/play":
                ...
                velocity = float(payload.get("velocity", 0.85))
                waveform = (payload.get("waveform") or "sine").lower()
                attack = float(payload.get("attack", 0.02))
                release = float(payload.get("release", 0.35))
                sample_rate = int(payload.get("sample_rate", 44100))
                buffer = self.vst_host.render_note(
                    note,
                    duration=duration,
                    velocity=velocity,
                    sample_rate=sample_rate,
                ...
                audio = encode_wav_bytes(buffer, sample_rate)
                encoded = base64.b64encode(audio).decode("ascii")
                self._send_json(
                    {
                        "ok": True,
